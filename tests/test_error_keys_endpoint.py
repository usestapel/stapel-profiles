...
``ProfilesErrorKeysView`` existed since the port but no ``urls*.py`` mounted
...
def test_route_is_mounted_and_resolves_to_the_view():
    ...
    assert resolve(path).func.view_class is ProfilesErrorKeysView
...
def test_staff_gets_the_registry_as_json():
    ...
    assert set(PROFILES_ERRORS) <= set(payload)
