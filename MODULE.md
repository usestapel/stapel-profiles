...
| `PROFILES_PUBLIC_FIELDS_ANONYMOUS` | identity + avatar + seller type | list \| `None` \| `["*"]` | What an UNAUTHENTICATED caller sees. **Narrow by default:** `user_id`, `display_name`, `avatar_source`, `avatar`, `avatar_image`, `seller_type` — no `location_*`, no follower/following counts, no `relationship_status` (which has no meaning without a viewer). `seller_type` rides with identity rather than with whereabouts/social-graph: it is a self-declared trading capacity a buyer is entitled to see before contacting, in most jurisdictions. Both public endpoints are `AllowAny`, so this is the answer the open internet gets. `None` or `["*"]` restores "anonymous callers see what members see". It may only ever narrow `PROFILES_PUBLIC_FIELDS` — a field hidden from members cannot reappear for the internet. |
